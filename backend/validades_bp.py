    ...
    try:
        ...
        for row in registros:
            ...
            produtos_formatados.append({
                ...
                "reservado": float(row.get('reservado') or 0.0)
            })
        return jsonify({"status": "success", "total": len(produtos_formatados), "data": produtos_formatados}), 200
    except Exception as e:
        logging.error(f"Erro ao buscar validades: {str(e)}")
        return jsonify({"status": "error", "message": "Erro ao carregar dados de validades.", "details": str(e)}), 500